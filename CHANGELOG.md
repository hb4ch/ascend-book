...
## 第15章 成稿（M3-7，实战Ⅱ：向量算子）

- **新增**：`ch15-vector-softmax.md`——softmax Case 0-5 六级阶梯逐级细讲 + GELU 双发实战 + 工程衔接（softmax_custom 入口/Tiling 消费/事件流水/AddrReg）。总 5.4k 字/中文 4.1k（计划 ≥4.5k 中文，实际 4.1k——质量优先，正式数据：Case 3+ 的 sumTensor 仅 8 字节、Case 2 的 exp 仍落 UB（全局归约检查点，寄存器容量物理上限）、GELU 三公式 13 指令/5 份内存→8/2。
- **配图 3 张**：ch15-ladder.svg（六级阶梯+官方 cycles：44113→1785，24.7×）、ch15-case1v2.svg（消灭的是冗余不是全部——修正初稿「全驻留」的过度声明）、图15-3 Mermaid（UpdateMask/主尾块决策）。
- **关键发现**：① Case 4 主尾块单独使用（3424）不比 Case 2（3413）快，收益与展开叠加才兑现；② Case 1→2 一步 9.3×（消灭冗余 UB 往返）；③ Hardware Loop 规范（uint16_t/从0步长1/循环内禁分支/for(1) 替代 if/成员变量外提否则 VF 融合失效）。
- **审查**：1 名子代理 4 维审查（溯源受限声明在案）——0 BLOCKER、4 MAJOR（3 处交叉引用错误+34% 口径）全修复、8 MINOR 修复 7 项（dav-3510 写法保留本书既定用法、图号物理顺序接受现状）。
- **还债①兑现**：11.5 预支（原定 ch14）的 RegTensor/VF 融合实操债本章还清。M3 实战系列进度 2/5。
